...
if (isset($_SESSION['fullname'])) {
    ...
    if (isset($_POST['submit1'])) {
        ...
        if ($checkindate > $checkoutdate) {
            header('Location:booking_y.php?id=1');
        }
    }
    if (isset($_POST['submit2'])) {
        ...
        $y = "Premium_room";
        $query = "SELECT * FROM booking WHERE type = '$y'";
        ...
        $query = "SELECT * FROM occupancy WHERE type = '$y'";
        $query1 = $dbhandler->query($query);
        $r = $query1->fetchAll();
        $tmp = 0;
        foreach ($r as $t){
        ...
        if($capacity == 2){
            if ($tmp-$tmp_2 >= $numofroom) {
                $query = "INSERT INTO Customer(name,phone_number,username,type,numofrooms,capacity,check_in_date,check_out_date) VALUES('$name','$phone_number','$username','$y','$numofroom','$capacity','$checkindate','$checkoutdate')";
                $query1 = $dbhandler->query($query);

                $query = "INSERT INTO booking(name,type,numofrooms,capacity,checkindate,checkoutdate) VALUES('$name','$y','$numofroom','$capacity','$checkindate','$checkoutdate')";
                $query1 = $dbhandler->query($query);

                header('Location:recipt_y.php?id=1');
            } else {
...
<body style="background-color: #2cabee;">
    ...
    <div class="container" style="background-color: #FFFFFF; border-radius: 15px; margin-top: 40px;padding: 80px;" align="center">
        ...
        <div style="width: 500px;">
            ...
            <form action="booking_y1.php" method="POST">
