...
if (isset($_SESSION['fullname'])) {
    ...
    if (isset($_POST['submit1'])) {
        ...
        if ($checkindate > $checkoutdate) {
            header('Location:booking_z.php?id=1');
        }
    }
    if (isset($_POST['submit2'])) {
        ...
        $z = "King_suite";
        $query = "SELECT * FROM booking WHERE type = '$z'";
        ...
        $query = "SELECT * FROM occupancy WHERE type = '$z'";
        $query1 = $dbhandler->query($query);
        $r = $query1->fetchAll();
        $tmp = 0;
        foreach ($r as $t){
        ...
        if($capacity == 2){
            if ($tmp-$tmp_2 >= $numofroom) {
                $query = "INSERT INTO Customer(name,phone_number,username,type,numofrooms,capacity,check_in_date,check_out_date) VALUES('$name','$phone_number','$username','$z','$numofroom','$capacity','$checkindate','$checkoutdate')";
                $query1 = $dbhandler->query($query);

                $query = "INSERT INTO booking(name,type,numofrooms,capacity,checkindate,checkoutdate) VALUES('$name','$z','$numofroom','$capacity','$checkindate','$checkoutdate')";
                $query1 = $dbhandler->query($query);

                header('Location:recipt_z.php?id=1');
            } else {
            ...
            if ($tmp-$tmp_3 >= $numofroom) {
                ...
                $booking_err = "<h5> King Suite has accomodation for 2 people only. </h5>";
            }
        }
    }
